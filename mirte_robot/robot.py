...
class Robot:
    ...
    def __init__(self) -> None:
        ...
        if rospy.has_param("/mirte/motor"):
            ...
            for motor in self.motors:
                self.motor_services[motor] = rospy.ServiceProxy(
                    "/mirte/set_" + self.motors[motor]["name"] + "_speed",
                    SetMotorSpeed,
                    persistent=True,
                )
        ...
        if rospy.has_param("/mirte/servo"):
            ...
            for servo in servos:
                self.servo_services[servo] = rospy.ServiceProxy(
                    "/mirte/set_" + servos[servo]["name"] + "_servo_angle",
                    SetServoAngle,
                    persistent=True,
                )
        ...
        if rospy.has_param("/mirte/distance"):
            ...
            for sensor in distance_sensors:
                self.distance_services[sensor] = rospy.ServiceProxy(
                    "/mirte/get_distance_" + distance_sensors[sensor]["name"],
                    GetDistance,
                    persistent=True,
                )
        ...
        if rospy.has_param("/mirte/oled"):
            ...
            for oled in oleds:
                self.oled_services[oled] = rospy.ServiceProxy(
                    "/mirte/set_" + oleds[oled]["name"] + "_image",
                    SetOLEDImage,
                    persistent=True,
                )
        ...
        if rospy.has_param("/mirte/intensity"):
            ...
            for sensor in intensity_sensors:
                if (
                    "/mirte/get_intensity_" + intensity_sensors[sensor]["name"]
                    in service_list
                ):
        ...
        if rospy.has_param("/mirte/encoder"):
            ...
            for sensor in encoder_sensors:
                self.encoder_services[sensor] = rospy.ServiceProxy(
                    "/mirte/get_encoder_" + encoder_sensors[sensor]["name"],
                    GetEncoder,
                    persistent=True,
                )
        ...
        if rospy.has_param("/mirte/keypad"):
            ...
            for sensor in keypad_sensors:
                self.keypad_services[sensor] = rospy.ServiceProxy(
                    "/mirte/get_keypad_" + keypad_sensors[sensor]["name"],
                    GetKeypad,
                    persistent=True,
                )
        ...
        self.set_pin_value_service: Callable[  # type: ignore[name-defined]
            [str, str, int], SetPinValueResponse
        ] = rospy.ServiceProxy("/mirte/set_pin_value", SetPinValue, persistent=True)
    ...
    def getDistance(self, sensor: str) -> float:
        ...
        Returns:
            float: Range in meters measured by the HC-SR04 sensor.
    ...
    def getIntensity(self, sensor: str, type="analog") -> int:
    ...
    def getAnalogPinValue(self, pin: Union[int, str]) -> int:
    ...
    def setAnalogPinValue(self, pin: Union[str, int], value: int) -> bool:
    ...
    def getDigitalPinValue(self, pin: Union[str, int]) -> bool:
    ...
    def setServoAngle(self, servo: str, angle: int) -> bool:
    ...
    def setDigitalPinValue(self, pin: Union[str, int], value: bool) -> bool:
